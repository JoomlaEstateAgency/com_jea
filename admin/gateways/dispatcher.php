...
class GatewaysEventDispatcher extends JEventDispatcher
{
            ...
            if ($this->_observers[$key] instanceof JeaGateway)
            {
                try {
                    ...
                    if ($application instanceof JApplicationCli) {
                        ...
                        // In CLI mode, output the error but don't stop the execution loop of other gateways
                        $gateway->out('Error [' . $gateway->title . '] : ' . $e->getMessage());

                    } else {
                        // In AJAX mode, only one gateway is loaded per request, so we can stop the loop.
    ...
    {
        $gatewayFile = JPATH_ADMINISTRATOR . '/components/com_jea/gateways/providers/'
                     . $gateway->provider . '/'. $gateway->type . '.php';
