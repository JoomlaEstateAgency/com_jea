...
class JFormFieldGallery extends JFormField
{
	...
	protected function getInput()
	{
		...
		{
			$output .= '<strong>WARNING: </strong>The <a href="http://php.net/manual/en/book.image.php" target="_blank">'
			...
			foreach ($images as $k => $image)
			{
				...
				if (! file_exists($imgBasePath . DS . $thumbName))
				{
					try
					{
						...
						{
							/*
							 * There is no destroy method on Jplatform < 12.3
							 * (Joomla 2.5) and the handle property is protected.
							 * We have to hack the JImage class to destroy the
							 * image resource
							 */
				...
				// TODO : Use JLayout here
				$output .= "<li class=\"item-$k\">\n"
						. "<a href=\"{$url}\" title=\"Zoom\" class=\"imgLink modal\" rel=\"{handler: 'image'}\">"
						. "<img src=\"{$thumbUrl}\" alt=\"{$image->name}\" /></a>\n"
						. "<div class=\"imgInfos\">\n" . $image->name . "<br />\n" . JText::_('COM_JEA_WIDTH') . ' : ' . $infos->width . ' px'
						. "<br />\n" . JText::_('COM_JEA_HEIGHT') . ' : ' . $infos->height . ' px' . "<br />\n" . "</div>\n"
						. "<div class=\"imgTools\">\n" . '  <a class="img-move-up" title="' . JText::_('JLIB_HTML_MOVE_UP') . '"><img src="'
						. $baseURL . '/media/com_jea/images/sort_asc.png' . '" alt="Move up" /></a>' . '  <a class="img-move-down" title="'
						. JText::_('JLIB_HTML_MOVE_DOWN') . '"><img src="' . $baseURL . '/media/com_jea/images/sort_desc.png" alt="Move down" /></a> '
						. '<a class="delete-img" title="' . JText::_('JACTION_DELETE') . '"><img src="' . $baseURL
						. '/media/com_jea/images/media_trash.png" alt="Delete" /></a>' . "</div>\n"
						. "<div class=\"clr\"></div>\n"
						. '<label for="' . $this->id . $k . 'title">' . JText::_('JGLOBAL_TITLE')
						. '</label><input id="' . $this->id . $k . 'title" type="text" name="' . $this->name . '[' . $k . '][title]" value="'
						. $image->title . '" size="20"/><br />'
						. '<label for="' . $this->id . $k . 'desc">' . JText::_('JGLOBAL_DESCRIPTION') . '</label><input id="' . $this->id . $k
						. 'desc" type="text" name="' . $this->name . '[' . $k . '][description]" value="' . $image->description . '" size="40"/>'
